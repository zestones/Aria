"""Sentinel reactive-detection loop (#24 / M4.2).

30-second tick that opens a ``work_order(status='detected')`` on threshold
breach, broadcasts ``anomaly_detected`` + ``ui_render(alert_banner)``, and
spawns the Investigator agent (#25) in the background.

The loop runs forever, started by the FastAPI lifespan (#26) alongside
:func:`agents.sentinel.forecast.forecast_watch_loop`. Each tick wraps its
body in ``try/except`` so a single bad cell cannot kill the entire
asyncio Task — the loop must survive any transient tool or DB error so
detection resumes on the next 30 s tick.
...
def _spawn_investigator(work_order_id: int) -> None:
    ...
    Fire-and-forget: the Investigator owns its own timeout + error
    envelope (see :mod:`agents.investigator.service`), so Sentinel never
    awaits the task.
    """
    asyncio.create_task(
