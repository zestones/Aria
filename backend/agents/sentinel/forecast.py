"""Forecast-watch predictive-alerting loop (M9).

Sibling to :func:`agents.sentinel.service.sentinel_loop`. Runs ordinary
least-squares regression on the tail of every monitored signal and emits
``forecast_warning`` on the events bus when the projected trajectory
will cross a threshold within :data:`_FORECAST_HORIZON_HOURS`.

Unlike Sentinel's breach detection — which only fires *after* a real
threshold crossing — this loop alerts the operator *before* the failure
condition occurs. Forecast warnings are ephemeral (WS-only), never
persist a row, and never open a work order: they are advisory. The
Investigator is not spawned from here.

Architecture, knob rationale and failure semantics are documented in
[docs/architecture/06-forecast-watch.md](../../../docs/architecture/06-forecast-watch.md).
"""
...
log = logging.getLogger("aria.forecast")

# Cadence is deliberately slower than Sentinel's 30 s breach loop:
# forecasts are trend-based and change on a much longer timescale than
# single-sample threshold crossings. A 60 s tick also halves DB pressure.
...
# (a tiny slope error compounds linearly with time); 12 h is the sweet
...
async def forecast_watch_loop() -> None:
    ...
    Starts on the same lifespan as
    :func:`agents.sentinel.service.sentinel_loop` (see :mod:`main`).
    Unlike Sentinel it does not need the MCP server to be up — it queries
    the DB directly — but we keep it inside the MCP lifespan block for
    symmetry with Sentinel's startup contract.
    """
    log.info(
        "Forecast-watch started (horizon=%dh, tick=%ds)",
        _FORECAST_HORIZON_HOURS,
        _FORECAST_TICK_SECONDS,
    )
    while True:
        try:
            await _forecast_watch_tick()
        except Exception:  # noqa: BLE001 — outer loop must survive any tick-level error
            log.exception("Forecast-watch tick failed — continuing")
...
async def _forecast_one_signal(
    *,
    ...
    # Severity tone: ETA within 2 h is trip-grade; otherwise an alert. The
