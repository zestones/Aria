...
function HeaderCell({ label, col, active, dir, onToggle }: HeaderCellProps) {
    ...
    const sort: "ascending" | "descending" | "none" = isActive
        ...
        : "none";
    return (
        <th className="border-b border-border font-medium" scope="col" aria-sort={sort}>
            <button
                type="button"
                onClick={() => onToggle(col)}
                className="inline-flex h-full w-full items-center gap-1 px-4 py-2.5 text-left font-medium text-muted-foreground transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
                <span>{label}</span>
...
function Row({ wo, onOpen }: { wo: WorkOrder; onOpen: () => void }) {
    ...
    return (
        <tr
            ...
            className="cursor-pointer border-b border-border transition-colors hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        >
            ...
            <td className="px-4 py-3 align-middle">
                <div className="flex min-w-0 flex-col gap-0.5">
                    <span className="truncate font-medium text-foreground">{wo.title}</span>
